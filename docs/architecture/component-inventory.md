...
| **ProfileTargets** | `/profile` preset + dials card (2026-07-14 remote-fit-criteria spec §8) | `{ profile: Profile; busy: boolean; onRelocationChange(v: RelocationPref): void; onScheduleChange(v: ScheduleFlex): void; onEmploymentChange(v: EmploymentPref): void; onPresetSelect(dials: { relocation; scheduleFlex; employmentPref }): void }` | Card×4 (preset row, `role="button"`), Chip×2/×3/×3 (three segmented pills, mirrors PersonaToggle), Select (baseCountry, single fixed option) | preset row "Which sounds like you?" (4 cards — Malaysia-only remote · Global remote · Digital nomad · Open to relocate — set all 3 dials at once; presets are not stored state) / relocation segmented (existing, 2: Stay in Malaysia · Open to relocate) / **schedule segmented (new, 3)** — "Malaysia hours" \| "Evenings OK — Europe overlap" \| "Any hours — US overlap" / **employment segmented (new, 3)** — "Any arrangement" \| "Employee — EOR OK" \| "Malaysian entity only" / busy (every control disabled, `aria-disabled`/`aria-pressed`) / load-error+retry |
