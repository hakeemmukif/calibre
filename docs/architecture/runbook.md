...
1. Create `.env` with:
   - `DATABASE_URL` — a Postgres connection string. Required just for the process to boot: `src/instrumentation.ts` calls `markStaleRunningOnBoot()` on startup, which calls `getDb()` (`src/server/persistence/db.ts`) — this throws `"DATABASE_URL is not set"` before **any** route, including `/api/health`, can serve a request.
...
2. Run `npm run db:migrate && npm run db:seed`. The seed rows (`src/server/persistence/seed.ts`) for `greenhouse`/`lever`/`ashby`/`jobstreet` ship with `REPLACE_ME` `config.slug`/`config.query` placeholders — replace them with real ATS org slugs / board queries before a real search can discover anything.
