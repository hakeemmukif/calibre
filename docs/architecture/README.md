...
UI (Storybook / app)
  └─ features/*  (typed service calls; status-map, feed assembly)
       └─ server/*  (the ONLY layer touching DB or LLM)
            ...
            └─ persistence   Drizzle (SQLite via libsql; embedded file locally, Turso-ready — §6 item 7 of system-architecture.md; migrated off Postgres 2026-07-16, see `2026-07-16-sqlite-migration-design.md`)
...
- **SQLite migration (2026-07-16):** persistence moved off Postgres — `server/persistence` is single-dialect SQLite via libsql (`drizzle-orm/sqlite-core`), embedded file-based in dev/tests (temp-file libsql instances) and Turso-ready in production. There is no PGlite test double. This supersedes any Postgres/PGlite reference elsewhere in this doc set (see `2026-07-16-sqlite-migration-design.md`).
- **Global postings pool cutover (2026-07-17):** the nightly crawl is now decoupled from user scans. `npm run crawl:once` (`src/server/sources/crawl.ts` → `runCrawl()` in `crawler.ts`) walks the enabled source connectors once/day and upserts into a shared, system-owned `postings` table (+ a `crawl_runs` row), independent of any request. `POST /api/search` no longer fans out to connectors per scan — it reads the pool via `postingsRepo.listForMatching(persona)` and hands the survivors to `server/score`. F2 in `system-architecture.md` §4 reflects this shipped flow (see `2026-07-17-global-postings-pool-architecture.md`).
