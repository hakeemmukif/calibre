...
describe("crawlRunsRepo.getLastCompletedFinishedAt", () => {
  it("returns the newest completed run's finishedAt, ignoring running/failed rows", async () => {
    ...
    await db.insert(crawlRuns).values({
      ...
      stats: { sourcesOk: 1, sourcesFailed: 0, perHostBackoffs: {}, upserts: 1, delists: 0, durationMs: 600_000, emptyFetches: [] },
    });
    await db.insert(crawlRuns).values({
      status: "completed",
    ...
    const finishedAt = await repo.getLastCompletedFinishedAt();
  ...
  it("returns null when no crawl has ever completed", async () => {
    const db = await createTestDb();
    const repo = createCrawlRunsRepo(db);
    expect(await repo.getLastCompletedFinishedAt()).toBeNull();
  });
});
...
describe("crawlRunsRepo.listFinishedRuns", () => {
  it("returns only non-running rows, newest first, limited", async () => {
    const db = await createTestDb();
    const repo = createCrawlRunsRepo(db);
    const stats = { sourcesOk: 1, sourcesFailed: 0, perHostBackoffs: {}, upserts: 1, delists: 0, durationMs: 1000, emptyFetches: [] };
