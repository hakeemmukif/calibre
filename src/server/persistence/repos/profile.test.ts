...
describe("profileRepo", () => {
  ...
  it("two users' profiles are independent (cross-tenant isolation)", async () => {
    ...
    await db.insert(profile).values([
      { id: "p-admin", userId: BOOTSTRAP_ADMIN_ID, baseCountry: "MY", relocation: "stay" },
      { id: "p-userb", userId: userBId, baseCountry: "SG", relocation: "open" },
    ]);
    const repo = createProfileRepo(db);
    ...
    await expect(repo.update(userBId, { baseCountry: "US", relocation: "stay" })).resolves.toMatchObject({ baseCountry: "US" });
  ...
  it("update(userId) flips relocation and bumps updatedAt", async () => {
    ...
    const updated = await repo.update(BOOTSTRAP_ADMIN_ID, { baseCountry: "MY", relocation: "open" });
  ...
  it("update(userId) throws ProfileMissingError when that user has no row", async () => {
    const db = await createTestDb();
    const repo = createProfileRepo(db);
    await expect(repo.update(BOOTSTRAP_ADMIN_ID, { baseCountry: "MY", relocation: "open" })).rejects.toBeInstanceOf(ProfileMissingError);
  });
  ...
  it("upsert() creates a row for a user with none, then updates it on a second call", async () => {
    ...
    const created = await repo.upsert(userBId, { baseCountry: "SG", relocation: "stay" });
    expect(created.userId).toBe(userBId);
    expect(created.baseCountry).toBe("SG");

    const updated = await repo.upsert(userBId, { baseCountry: "SG", relocation: "open" });
  ...
  it("the seeded admin's first upsert() does not raise a unique violation (conflict-target regression guard)", async () => {
    ...
    const result = await repo.upsert(BOOTSTRAP_ADMIN_ID, { baseCountry: "SG", relocation: "open" });
  ...
  it("upsert() for two different users never conflicts with each other", async () => {
    ...
    await repo.upsert(BOOTSTRAP_ADMIN_ID, { baseCountry: "MY", relocation: "stay" });
    await repo.upsert(userBId, { baseCountry: "SG", relocation: "open" });
