...
```bash
# 1. isolated scratch DB (never touch the operator's dev DB)
psql -d postgres -c "DROP DATABASE IF EXISTS caliber_verify" -c "CREATE DATABASE caliber_verify"
DATABASE_URL="postgresql://localhost:5432/caliber_verify" npm run db:migrate
DATABASE_URL="postgresql://localhost:5432/caliber_verify" npm run db:seed:test   # seeds sources (incl. `manual`) + a `default` profile, NO résumé

# 2. boot the built app against it (run in background). instrumentation.ts starts the url-check worker on boot.
DATABASE_URL="postgresql://localhost:5432/caliber_verify" CALIBER_TEST_DOUBLES=1 npx next start -p 3123   # needs a prior `next build`
...
- **Concurrency cap (=3):** POST ~8 jobs at once, then tight-loop `psql -d caliber_verify -tc "select count(*) from url_checks where status='running'"` and record the peak — must be ≤3.
...
- Cleanup: `pkill -f "next start -p 3123"`; `psql -d postgres -c "DROP DATABASE IF EXISTS caliber_verify"`.
